...
public class FakeServerAdapter implements ConfigurationAdapter {
    private static final Map<String, ServerInfo> fakeServers = new HashMap<>();
    private static FakeServerAdapter instance = null;

    public static void inject(ProxyServer server) {
        if (instance == null) {
            instance = new FakeServerAdapter(server.getConfigurationAdapter());
        }
    ...
    public FakeServerAdapter(ConfigurationAdapter adapter) {
